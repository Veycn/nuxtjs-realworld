...
Object.keys(swaggerJson.paths).forEach(path => {
    let pathObj = swaggerJson.paths[path]
    Object.keys(pathObj).forEach(method => {
        let key = pathObj[method]['operationId']
        if (!key) {
            key = pathObj[method]['summary'].replace(/\b(\w)|( \w)/g, w => w.toUpperCase().trim())
        }
        api[key] = method + ':' + swaggerJson.basePath + path
    })
})
